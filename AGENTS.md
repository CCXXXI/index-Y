...
- `X/`：指向 `index-X/EPUB` 的目录联接（Windows junction / POSIX symlink，gitignored 不入库）。文件系统口径的 X，读取透明；不存在时由脚本 preflight（`ensure_x`）自动创建。git 操作一律走 `index-X`，不走 `X/`。
...
不变式：**X 是上游逐字镜像（index-X@pin 的 `EPUB/` 树），Y 是 `x2y(X)` 净化产物。**
