...
- delegate_task 分派事故（S1_04 犯过）：主代理连续多次调用漏写 tasks 项的 `context`（响应回显只有 goals，发现不了），子 agent 裸 goal 起跑。分派后读一只 live transcript 的 kickoff 验证子 agent 实际收到模板；补救用 `delegate_task(action='steer')` 逐只补发「加载模板 + VOL_PREFIX + 严格 JSON」可行（子 agent 多会自行 skill_view 模板但不可依赖）。
