...
- 成本参考（S1_01，按章分派旧方案）：11 万字符卷 56 发现、误报 <15%、6 agent 31 分钟；子 agent 原文回查接近遇疑必查（全卷 164 词），订阅额度按百万 token/卷估。切块 + 批量查询后墙钟应压到 ~15 分钟级。S1_02 实测（切块新方案）：12 万字符卷 9 块并行，最慢块 16.6 分钟，54 发现采纳 55 条规则（57 处修订）、否决 1 条。
