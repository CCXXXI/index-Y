...
- 错字短旧串防「正用」误伤（S1_07 头捶案）：「头捶→头槌」若作旧串会把 S2_01「用拳头捶在墙上」的正用子串绞成「头槌在墙上」——两字错字规则先全语料看命中形态，有正用就带上下文加长旧串（愤怒的头捶）。归属计数会挡，预查省一轮裁决。
- 候选 TSV 漏抄对账（S1_07 沉思者案）：主代理手写 40 行 TSV 把已裁定的「沉思者→思想者」漏抄，apply_rules 全过发现不了——靠「EPUB numstat 行数 vs 采纳清单条数」对账抓获（Ch4 预期 6 行实得 5 行）。写 TSV 后先对行数（= 采纳规则数），落盘后再对 numstat，两道都过才算完。
