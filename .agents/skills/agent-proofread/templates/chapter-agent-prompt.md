...
3. 输出含译文逐字上下文与日文原文命中（已剥注音），其中目标卷 X/Y 上下文覆盖全卷，也可用于核对你区间外的一致性。引用日文必须照抄脚本输出的实际句子，禁止凭记忆编造日文原文；jp_evidence 只写日文句子本身——定位器输出尾部的（partXXXX.html）来源标注不要写进去，中文解说写进 issue 字段。
