...
- 先把**旧 X 与旧 Y 的文本块序列做位置对齐**（1:1，允许旧 Y 被规则改写），把两侧改动配成「同位置块对」；对齐失败 → 复杂，整文件留下。规则的替换文本可能注入/消除带文本的标记（如 `<ruby><rt>` 注音），使 Y 侧块数与 X 不等：此类增删块在 HEAD 与工作中等量存在、不影响对齐，**HEAD 上 fixed(X) == Y 成立时容忍**（证明增删是规则产物），否则才算真复杂。必须按位置配对而不能只做全局片段集比较：同一最小片段（如 `冷气→空调`）可能一处是两侧共享改动、另一处是上游采纳规则，全局比较会把后者误判为正常同步。
- **结构改动文件**（单侧出现块增删或 N→M 替换，如上游插入译者注/特典条目、拆分合并段落）走块组级配对：以「映射后旧块锚点 + 操作类型 + 组长度」配对两侧 difflib 组，配对组做内容收敛验证（X 组文本应用规则后 == Y 组文本），1:1 组逐块走下方块级分类。全部块对为 sync/rulekilled 时成对提交（提交前校验换行归一化后 fixed(X) == Y），任一疑似块 → 整文件留审。排除粒度与普通文件一致：`triage_exclude.json` 命中的 1:1 块在两侧工作区文本上**还原回 HEAD 旧文本后提交中间版本**（同一文本多处出现且出现次数与排除次数相等时全部还原；过短无法唯一定位的块按「HEAD→中间版的 1:1 replace 中唯一匹配」做块序定位；还原后校验两侧块多重集差异恰好是被排除对）；被排除的是结构组（增删/N→M 无法按块还原）时才整文件留审。
- 逐块对比较**最小差异片段**集 F(x) 与 F(y)：F(x) == F(y) → 正常同步候选对；F(x) ⊃ F(y) 且 X 多出的片段能被 x2y.py 规则解释（收敛判定：反复应用「能严格缩小块差异」的规则后，剩余差异等于 F(y)）→ **混合块对**：Y 块整块提交，X 侧**片段级拆分**——正常同步片段照常提交、规则采纳片段留下（`apply_blocks` 的 `partial` 参数在 raw 块内把规则采纳的新片段换回旧片段；片段定位失败退化为整块还原）。这样两侧提交保持对称，HEAD 上 x2y(X) == Y 的不变式也不被破坏；仅 X 有改动且收敛 → 规则采纳；Y 侧多出片段时，若 fixed 能分别从新旧 X 文本得到新旧 Y 文本（规则因上游编辑获得触发语境，如编辑引入「所以」触发 `由于→因为`），两侧差异纯属规则渲染 → 正常同步；其余（仅 Y 有改动、不收敛、HEAD 上该块两侧已有规则解释不了的不对称）→ 疑似上游错误留下。片段级比较是必要的：旧 Y 的同一句子可能已被 x2y 规则改过（如 `其它→其他`），块字面不同不代表改动不同。
...
**提交粒度为块**：`apply_blocks(HEAD, 工作区, keep=通过的块, partial=混合块)` 构造「只应用正常同步改动」的中间版本提交，规则采纳/疑似块与被人工排除的块自动留在工作区。同一文件中混合的正常同步与其他块因此可以分离——可疑块不影响同文件其余正常块的提交。每文件仍是一个 commit（X/Y 成对），提交信息 `fix: sync <相对路径>（N 处文本修订）` 的 N 只计实际提交的正常同步块。

排除粒度同样是块：`triage_exclude.json` 中被排除的块留下，同文件其余正常同步块照常提交。

### 正常同步候选的正常性判断

上游同步的文本改动绝大多数是：术语统一（如 `欧陆隧道→英法海底隧道`、`宗主教→总大主教`）、错别字（`闹区→闹市区`、`脚裸→脚踝`）、的地得、标点全半角、人名译名统一、以及整段流畅的重译。这些都算正常。

需要**剔除留审**的异常特征（附案例）。上游的错误改动一般是**批量替换未经充分检查**的机械产物；人工针对单处的语义/语气调整（即使含义反转、措辞大变）是有意修订，不算错误：

- 明显退化：`瞠目结舌→得目结舌`、`正面相对的→正面相对地`（的地误用方向反了）；
- 重复字词/标点/杂字符：`一条很长的一条道`、`开了口。开了口。`、`现在。。。`、混入孤立 `>`、`「「`；
- 删字导致病句：删掉「瞄准」使「把唯一的将帅、国王。」不通；
- 莫名其妙的名词替换（批量替换误伤）：`麦野沉利→麦野沈利`（抄日文汉字原形未转简体）；
- 数字、专名改动且无规律可循。

仅凭中文难以判断的，对照日文原文（本地私有仓库 index-jp）再定夺：看似莫名的改动可能是原文的忠实再现（如 `哈呀→八百` 对应「はっぴゃ!?」、`嵌入式垃圾桶→洽奈儿垃圾桶` 对应专名「チャネル」）。
...
- **s6a**：按块提交规则相关的自动分流块——规则采纳块提交 X 侧
  （`fix: sync X/<rel>（上游采纳规则，N 处）`），规则失效型收敛块成对
  提交（`fix: sync <rel>（N 处文本修订）`）。s6b 候选规则导出为累积制
