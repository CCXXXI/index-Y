...
- **结构改动文件**（单侧出现块增删或 N→M 替换，如上游插入译者注/特典条目、拆分合并段落）走块组级配对：以「映射后旧块锚点 + 操作类型 + 组长度」配对两侧 difflib 组，配对组做内容收敛验证（X 组文本应用规则后 == Y 组文本），1:1 组逐块走下方块级分类。块组无法像普通块那样按块拆分提交，因此**全部块对为 sync/rulekilled 且无排除时整文件成对提交**（提交前校验换行归一化后 fixed(X) == Y），任一疑似/被排除块 → 整文件留审。结构组的增删块同样导出到 review_changes.txt 供审查（排除按块对元组生效，命中则整文件留审）。
- 逐块对比较**最小差异片段**集 F(x) 与 F(y)：F(x) == F(y) → 正常同步候选对；F(x) ⊃ F(y) 且 X 多出的片段能被 x2y.py 规则解释（收敛判定：反复应用「能严格缩小块差异」的规则后，剩余差异等于 F(y)）→ **混合块对**：Y 块整块提交，X 侧**片段级拆分**——正常同步片段照常提交、规则采纳片段留下（`apply_blocks` 的 `partial` 参数在 raw 块内把规则采纳的新片段换回旧片段；片段定位失败退化为整块还原）。这样两侧提交保持对称，HEAD 上 x2y(X) == Y 的不变式也不被破坏；仅 X 有改动且收敛 → 规则采纳；其余（Y 有多余片段、仅 Y 有改动、不收敛）→ 疑似上游错误留下。片段级比较是必要的：旧 Y 的同一句子可能已被 x2y 规则改过（如 `其它→其他`），块字面不同不代表改动不同。
