...
按相对路径配对 `X/<rel>` 与 `Y/<rel>`（两侧目录结构相同），各自提取相对 HEAD 的文本改动（文本块级 difflib，仅接受干净的 1:1 replace），然后**以改动块为最小单位**分类（不再整文件一刀切）：

- 先把**旧 X 与旧 Y 的文本块序列做位置对齐**（1:1，允许旧 Y 被规则改写），把两侧改动配成「同位置块对」；对齐失败 → 复杂，整文件留下。只做全局片段集比较而不按位置配对，会把「同碎片的另一处规则采纳改动」误判为类别 1（2026-08 实测：`冷气→空调` 一处是共享改动、另一处是上游采纳规则，全局片段子集判定把后者也当成了类别 1）。
...
**提交粒度为块**：`apply_blocks(HEAD, 工作区, keep=通过的块)` 构造「只应用类别 1 块」的中间版本提交，类别 2/3 块与被人工排除的块自动留在工作区。同一文件中混合的类别 1 与类别 2/3 因此可以分离——一个可疑块不再拖累整文件的正常改动。每文件仍是一个 commit（X/Y 成对），提交信息 `fix: sync <相对路径>（N 处文本修订）` 的 N 只计实际提交的类别 1 块。
