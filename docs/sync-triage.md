...
按相对路径配对 `X/<rel>` 与 `Y/<rel>`（两侧目录结构相同），提取各自相对 HEAD 的文本改动集（文本块级 difflib，仅接受干净的 1:1 replace）：

- **改动集比较要在「最小差异片段」级进行**，不要直接比较整个文本块：旧 Y 的同一句子可能已被 x2y 规则改过（如 `其它→其他`），导致同一处上游改动在两侧的文本块字面不同而被误判为「不一致」。对块内做 difflib 取 replace 片段集合再比较即可正确判等。

| 情况 | 分类 | 处理 |
|---|---|---|
| 两侧改动集完全相同 | 候选类别 1 | 逐条人工/AI 审查改动内容，全部正常 → `git commit -- X/<rel> Y/<rel>` |
| 两侧改动集不同 | 疑类别 2 | 留下 |
| 仅 X 有改动 | 类别 3 | 留下（之后提交 + 注释 x2y.py 规则） |
| 任一侧对齐失败（文本增删等） | 复杂 | 留下 |
